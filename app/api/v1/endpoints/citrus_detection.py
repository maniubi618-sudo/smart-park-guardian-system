...
router = APIRouter(prefix="/citrus-detection", tags=["柑橘成熟度检测"])
...
async def detect_citrus(file: UploadFile = File(...)):
    """
    上传图片进行柑橘成熟度检测
    """
    try:
        detector = get_citrus_detector()
        
        if detector.model is None:
            return DetectionResponse(
                success=False,
                predictions=[],
                message="模型未加载，请确保 citrus_maturity.pt 文件存在"
            )
        ...
        return DetectionResponse(
            ...
            message=f"检测到 {len(predictions)} 个柑橘"
        )
